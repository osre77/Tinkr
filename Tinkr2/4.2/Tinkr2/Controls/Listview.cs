    ...
    public class Listview : Control
    {
        ...
        protected override void TouchDownMessage(object sender, point e, ref bool handled)
        {
            ...
            if (_cols != null && e.Y  < _header.Height + 9)
            {
                ...
                {
                    bounds = new rect(_cols[i]._x, 1, _cols[i]._w, _header.Height + 8);
        ...
        {
            if (_cols == null)
                _cols = new ListviewColumn[] { column };
            else
            {
                ListviewColumn[] tmp = new ListviewColumn[_cols.Length + 1];
        ...
        {
            if (_items == null)
                _items = new ListviewItem[] { line };
            else
            {
                ListviewItem[] tmp = new ListviewItem[_items.Length + 1];
        ...
        public void AddLines(ListviewItem[] lines)
        {
            ...
            {
                ListviewItem[] tmp = new ListviewItem[_items.Length + lines.Length];
                Array.Copy(_items, tmp, _items.Length);
        ...
        protected override void OnRender(int x, int y, int w, int h)
        {
                ...
                if (_items != null)
                {
                    for (i = 0; i < _items.Length; i++)
                    {
                        ...
                        if (y + h > 0)
                        {
                            ...
                            if (_items[i]._values != null)
                            {
                                c = (_items[i]._values.Length < _cols.Length) ? _items[i]._values.Length : _cols.Length;
                                for (j = 0; j < c; j++)
                                {
                                    w = _cols[j].Width;
                                    if (w == -1)
                                        w = _autoW;
                                    Core.Screen.DrawTextInRect(_items[i]._values[j], x + 4, y + 4, w - 8, _item.Height, Bitmap.DT_TrimmingCharacterEllipsis, (i == _iSel) ? _selTextC : _itemC, _item);
                                    x += w;
                                }
                            }
                        }
            ...
            if (_bMoved && _items != null)
            {
                ...
                int _lineHeight = _item.Height + 8;
                int msa = (_items.Length * _lineHeight) - (Height - _header.Height - 16);
                int iGripSize = (Height - _header.Height - 16) - (msa / _lineHeight);
                if (iGripSize < 8)
                    iGripSize = 8;

                int iOffset = Top + _header.Height + 14 + (int)(((Height - _header.Height - 16) - iGripSize) * ((float)_scrollY / (float)msa));
