...
namespace Skewworks.NETMF.Controls
{
    ...
    public class Scrollbar : Control
    {
        ...
        private Bitmap _up = Resources.GetBitmap(Resources.BitmapResources.up);
        private Bitmap _down = Resources.GetBitmap(Resources.BitmapResources.down);
        private Bitmap _left = Resources.GetBitmap(Resources.BitmapResources.left);
        private Bitmap _right = Resources.GetBitmap(Resources.BitmapResources.right);
        ...
        protected override void TouchMoveMessage(object sender, point e, ref bool handled)
        {
            ...
            if (_orientation == Orientation.Horizontal)
                Value = (int)((_max - _min) * ((float)((_grpRect.X + (e.X - _mX)) - Left - 16) / (float)(Width - 32 - _grpRect.Width))) + _min;
            else
                Value = (int)((_max - _min) * ((float)((_grpRect.Y + (e.Y - _mY)) - Top - 16) / (float)(Height - 32 - _grpRect.Height))) + _min;
        ...
        protected override void OnRender(int x, int y, int w, int h)
        {
            ...
            if (_orientation == NETMF.Orientation.Vertical)
            {
                if (Enabled)
                {
                ...
                iOffset = y + 16 + (int)((Height - 34 - iGripSize) * ((float)(_val - _min) / (float)(_max - _min)));
            ...
            else
            {
                ...
                iOffset = x + 16 + (int)((Width - 34 - iGripSize) * ((float)(_val - _min) / (float)(_max - _min)));
