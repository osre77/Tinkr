...
namespace Skewworks.NETMF.Controls
{
    ...
    public class Textbox : Control
    {
        ...
        protected override void OnRender(int x, int y, int w, int h)
        {
            if (Core.FlatTextboxes)
            {
                if (Enabled && !_bReadonly)
                    Core.Screen.DrawRectangle(0, 0, Left, Top, Width, Height, 0, 0, Core.SystemColors.WindowColor, 0, 0, Core.SystemColors.WindowColor, 0, 0, 256);
                else
        ...
        protected override void KeyboardKeyMessage(char key, bool pressed, ref bool handled)
        {
            ...
            {
                handled = true;
                InsertStrChr(new string(new char[] { key }));
                Invalidate();
            }
            else if (key == 8)
            {
        ...
        {
            TextEditorArgs args = new TextEditorArgs(_editorFont, _editorTitle, _text);
            OnTextEditorStart(this, ref args);
            VirtualKeyboard VK = new VirtualKeyboard();
            VK.TextEditorClosing += new Controls.OnTextEditorClosing(VK_TextEditorClosing);
