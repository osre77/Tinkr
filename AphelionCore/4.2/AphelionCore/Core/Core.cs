...
namespace Skewworks.NETMF
{
    ...
    public class Core : MarshalByRefObject
    {
        ...
        private int _sW, _sH, _sBPP, _sRot;

        // Touch
        private SafeTouchConnection _safetouch;
        private Thread myTouch;

        // Controls
        private IContainer _active;
        private SystemColors _colors;
        ...
        // Mouse
        private Bitmap _mouse;
        private Bitmap _mouseBuffer;
        ...
        // Messaging
        private static MessageClient _mc = new MessageClient();
        ...
        private Core(TouchCollection touchCollection, Bitmap splash, Color backColor)
        {
            ...
            if (touchCollection == TouchCollection.NativeSingleTouch)
            {
                ...
                TouchCollectorConfiguration.CollectionMode = Microsoft.SPOT.Touch.CollectionMode.InkOnly;
                ...
                myTouch.Priority = ThreadPriority.AboveNormal;
                myTouch.Start();
            }
        ...
        public static point MousePosition
        {
            ...
            set
            {
                ...
                int rx, ry, rw, rh;
                rx = System.Math.Min(value.X, _instance._mouseX);
                ry = System.Math.Min(value.Y, _instance._mouseY);
                rw = System.Math.Abs(value.X - _instance._mouseX) + 12;
                rh = System.Math.Abs(value.Y - _instance._mouseY) + 19;
        ...
        {
            _instance.BroadcastMessage(sender, message, args, _mc);
        }
        ...
        public static void ClipForControl(IControl Control, int X, int Y, int Width, int Height)
        {
            ...
            {
                IControl c = (IControl)Control.Parent;
                r = c.ScreenBounds;
            }
        ...
        private bool launchApp(string filename, string[] args = null)
        {
            ...
            try
            {
                ...
                if (ILauncher.LoadApp(_instance, filename, GetDependencies(filename)))
                {
                    if (_appRefs == null)
                        _appRefs = new ApplicationLauncher[] { (ApplicationLauncher)ILauncher };
                    else
                    {
                        ApplicationLauncher[] tmp = new ApplicationLauncher[_appRefs.Length + 1];
                        Array.Copy(_appRefs, tmp, _appRefs.Length);
                        tmp[tmp.Length - 1] = (ApplicationLauncher)ILauncher;
                        _appRefs = tmp;
                    }

                    OnApplicationLaunched(this, ILauncher.ThreadId);
                    return true;
                }
            }
            catch (Exception)
        ...
        private bool launchApp(byte[] appData, string[] args = null)
        {
            ...
            try
            {
                ...
                if (ILauncher.LoadApp(_instance, appData))
                {
                    if (_appRefs == null)
                        _appRefs = new ApplicationLauncher[] { (ApplicationLauncher)ILauncher };
                    else
        ...
        public static bool ShellNETMF(string peFilename, string Domain)
        {
            ...
            IApplicationLauncher ILauncher = (IApplicationLauncher)AD.CreateInstanceAndUnwrap(typeof(IApplicationLauncher).Assembly.FullName, typeof(ApplicationLauncher).FullName);
            bOK = ILauncher.ShellApp(_instance, peFilename, GetDependencies(peFilename));
            AppDomain.Unload(AD);
            return bOK;
        }
        ...
        public static bool ShellNETMF(byte[] peData, string Domain)
        {
            ...
            AppDomain AD = AppDomain.CreateDomain(Domain);
            IApplicationLauncher ILauncher = (IApplicationLauncher)AD.CreateInstanceAndUnwrap(typeof(IApplicationLauncher).Assembly.FullName, typeof(ApplicationLauncher).FullName);
            bOK = ILauncher.ShellApp(_instance, peData);
            AppDomain.Unload(AD);
            return bOK;
        }
        ...
        {
            ushort[] alpha = new ushort[] { 76, 51, 26, 13 };
            int i;
            int v1 = 0;
            int v2 = 1;
            int v3 = 2;
        ...
        /// </summary>
        /// <param name="Application">Registered application to terminate</param>
                ...
                for (int i = 0; i < _appRefs.Length; i++)
                {
                    if (_appRefs[i].ThreadId.Equals(threadId))
                    {
                        try
                        {
                            ...
                            {
                                IApplicationLauncher[] tmp = new IApplicationLauncher[_appRefs.Length - 1];
                                int c = 0;
        ...
        private void raiseTouch(TouchType touchType, point pt, float force = 1f)
        {
        ...
        {
            if (_mcs == null)
                _mcs = new MessageClient[] { mc };
            else
            {
                MessageClient[] tmp = new MessageClient[_mcs.Length + 1];
        ...
        private class ApplicationLauncher : MarshalByRefObject, IApplicationLauncher
        {
            ...
            public bool ShellApp(Core instance, string filename, string[] Dependencies)
            {
                ...
                // Set Core Instance
                Core.Host = instance;
                instance.AddMessageClient(Core.MessageClient);
                ...
                {
                    Debug.Print("Launch Application Error: " + ex.ToString());
                }
