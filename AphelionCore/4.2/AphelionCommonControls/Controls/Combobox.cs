...
namespace Skewworks.NETMF.Controls
{
    ...
    public class Combobox : Control
    {
        ...
        protected override void ButtonReleasedMessage(int buttonID, ref bool handled)
        {
            if (buttonID == (int)ButtonIDs.Select && _expand)
            {
                ...
                else
                {
                    ...
                    Listbox lstDynamic = new Listbox(Name + "_lst", _font, X, Y + Height - 1, Width - 31, h, _items);
                    lstDynamic.ZebraStripe = _bZebra;
                    lstDynamic.ZebraStripeColor = _zebra;
                    lstDynamic.SelectedIndex = _selIndex;
                    lstDynamic.DoubleTap += (object sender2, point e2) => CleanUp(lstDynamic);
                    lstDynamic.LostFocus += (object sender3) => CleanUp(lstDynamic, false);
                    this.Parent.AddChild(lstDynamic);
                    this.Parent.ActiveChild = lstDynamic;
                }
            }
            ...
            if (_items != null)
            {
                if (e.X > Width - 31 && _expand)
                {
                    ...
                    else
                    {
                        ...
                        lstDynamic.ZebraStripeColor = _zebra;
                        lstDynamic.SelectedIndex = _selIndex;
                        lstDynamic.Tap += (object sender2, point e2) => CleanUp(lstDynamic);
                        lstDynamic.LostFocus += (object sender3) => CleanUp(lstDynamic, false);
                        this.Parent.AddChild(lstDynamic);
                        this.Parent.ActiveChild = lstDynamic;
                    }
        ...
        public void AddItem(ListboxItem item)
        {
            // Update Array Size
            if (_items == null)
                _items = new ListboxItem[] { item };
            else
            {
                ListboxItem[] tmp = new ListboxItem[_items.Length + 1];
        ...
        public void AddItems(ListboxItem[] items)
        {
            ...
            {
                ListboxItem[] tmp = new ListboxItem[_items.Length + items.Length];
                Array.Copy(_items, tmp, _items.Length);
        ...
        protected override void OnRender(int x, int y, int w, int h)
        {
            ...
            // Draw Fill
            if(Enabled)
                Core.Screen.DrawRectangle(0, 0, Left + 1, Top + 1, Width - 3, Height - 3, 0, 0, Core.SystemColors.ControlTop, Left, Top, Core.SystemColors.ControlBottom, Left, Top + (Height / 2), 256);
            else
